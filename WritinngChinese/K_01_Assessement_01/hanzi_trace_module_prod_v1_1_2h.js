  ...
  function mount(container, options){
    ...
    function endPress(){
      ...
      try{ const wasLast=(current===basePaths.length-1);
        if(wasLast){ const ev=new CustomEvent('hanzi:last-stroke-complete',{detail:{char:opts.char||''}});
          window.dispatchEvent(ev); } }catch(e){} setTimeout(()=>{ current=Math.min(current+1, basePaths.length-1); recolor(); updateDotVisibility(); }, opts.nextDelayMs|0); }
