...
(function(global){
  ...
  function mount(container, options){
    ...
    function makeLightStrip(pos){
      ...
      // 14 lights on top/bottom, 13 on left/right to wrap the border
      const isHorizontal = (pos === "top" || pos === "bottom");
      const count = isHorizontal ? 14 : 13;

      if (pos === "top") {
        strip.style.left = "50%";
        strip.style.transform = "translateX(-50%)";
        // move slightly above the miZiGe border
        strip.style.top = "-12px";
        strip.style.flexDirection = "row";
      } else if (pos === "bottom") {
        strip.style.left = "50%";
        strip.style.transform = "translateX(-50%)";
        // move slightly below the miZiGe border
        strip.style.bottom = "-12px";
        strip.style.flexDirection = "row";
      } else if (pos === "left") {
        strip.style.top = "50%";
        strip.style.transform = "translateY(-50%)";
        // move slightly to the left of the miZiGe border
        strip.style.left = "-12px";
        strip.style.flexDirection = "column";
      } else if (pos === "right") {
        strip.style.top = "50%";
        strip.style.transform = "translateY(-50%)";
        // move slightly to the right of the miZiGe border
      ...
      for (let i = 0; i < count; i++) {
        ...
        b.style.opacity = "0.3";
        b.style.transition = "transform 0.25s ease-out, box-shadow 0.25s ease-out, opacity 0.25s ease-out";
    ...
    function startGlow(){
      ...
      glowTimer = setInterval(() => {
        lightBubbles.forEach(el => {
          ...
          if (big) {
            // all lights glow big and bright
            el.style.transform = "scale(1.5)";
            el.style.boxShadow = "0 0 26px " + c;
            el.style.opacity = "1";
          } else {
            // slightly smaller but still glowing
            el.style.transform = "scale(1.1)";
            el.style.boxShadow = "0 0 16px " + c;
  ...
  updateDotVisibility();
  // turning lights off whenever undo is pressed
  setLights(false);
