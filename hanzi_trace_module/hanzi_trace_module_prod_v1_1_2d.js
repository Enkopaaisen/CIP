...
/* Hanzi Trace Module v1.1.2d – Show Dots Always for Active Stroke
 * Fix: The "Show Dots" button now ALWAYS turns on dots + numbers
 * for the CURRENT (active) stroke — even if that stroke is already completed.
 * (Removes the previous check that hid dots when a stroke was completed.)
 * Also keeps 10% fit-in-grid and the Undo/Clear/Save logic.
 */
(function(global){
  'use strict';
  ...
  function mount(container, options){
    ...
    function pointWithinStroke(si,x,y){
      ...
      ctx.save();
      ctx.translate(TX,TY);
      ctx.scale(S,(opts.flipY?-S:S));
      const path=basePath2D[si];
      const inside=ctx.isPointInPath(path, x, y);
      let near=false;
      if(!inside && opts.strokeTol>0){ ctx.lineWidth=opts.strokeTol*2; near=ctx.isPointInStroke(path, x, y); }
      ctx.restore();
      return inside||near;
    }
    ...
    function showDotsForActiveAlways(){
      ...
      dotEls.forEach((arr, idx) => (arr||[]).forEach(el => {
        const on = (idx === si);         // ALWAYS show for active stroke, even if completed
        el.halo.style.display='none';    // halo off
    ...
    function updateDotVisibility(){
      ...
      // For "active" or "always", we still only show the active stroke's dots,
      // but do not include the halo and do not suppress if completed.
    ...
    // Button handlers
    btnShowDots.onclick = ()=>{ opts.showDots = "active"; showDotsForActiveAlways(); };
