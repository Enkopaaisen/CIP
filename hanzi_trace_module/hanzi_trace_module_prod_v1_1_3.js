...
/*! Hanzi Trace Module v1.1.3 (PROD)
 *  Minimal mount-only API for embedding in a character card.
 *  Auto-loads <char>_settings.json (optional), <char>_strokes.json, <char>_dots.json via basePath.
 *  Shows only: grid + 3 icon buttons (Undo, Clear, Save).
 */
(function(global){
  'use strict';
  ...
  function mount(container, options){
    ...
    // Container skeleton (only grid + 3 icon buttons)
    container.innerHTML = "";
    ...
    clearBtn.onclick = ()=>{ gTrace.innerHTML=""; tracePaths = tracePaths.map(()=>[]); hitSets = dots.map(()=> new Set()); current=0; recolor(); updateDotVisibility(); };
    
// === Injected helpers: Save trace to small 120x120 boxes (sg1..sg5) ===
...
function _drawToSmallBox(canvasOrImage, targetCanvas){
  ...
  ctx.restore();
  const pad = 6, dw = W - pad*2, dh = H - pad*2;
...
saveBtn.onclick  = ()=>{
  ...
  if (boxes && boxes.length){
    _snapshotTraceSVG(svg).then((snap)=>{
  ...
  // Fallback to original PNG download if no boxes present
  ...
  img.src = url;
    ...
    else { initFromFetch().catch(err=>{ console.warn("HanziTraceModule: failed to load data for", opts.char); layout([]); setDots(null); recolor(); }); }
