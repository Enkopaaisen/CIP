...
/*! Hanzi Trace Module – Shim v1.1.3c
 * Targets buttons by aria-label/title:
 *   - [aria-label="Save image"] (or title="Save image")
 *   - [aria-label="Undo last trace"] (or title="Undo last trace")
 * Intercepts Save to write into sg1..sg5; after Undo, restores dot/label visibility.
...
(function(){
  ...
  function snapshotSVGToCanvas(svgEl){
    ...
    style.textContent=".grid line,.grid rect{stroke:#cfcfcf;stroke-width:1.2;fill:none}.grid .border{stroke-width:1.5}.trace{fill:none;stroke:#000;stroke-linecap:round;stroke-linejoin:round}.dotHalo{fill:rgba(76,175,80,.10);stroke:rgba(76,175,80,.25)}.dot{stroke:#333;stroke-width:1}.dotLabel{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;font-weight:700;text-anchor:middle;dominant-baseline:middle;paint-order:stroke;stroke:#fff;stroke-width:2px;fill:#111;}";
  ...
  function saveToBoxes(){
    ...
    snapshotSVGToCanvas(svg).then(function(snap){
  ...
  function forceDotsVisible(){
    ...
    for(var i=0;i<dots.length;i++){ var el=dots[i]; el.style.display=''; el.style.opacity='1'; el.style.visibility='visible'; }
  }

  ready(function(){
    var root=document.getElementById('tracegrid')||document;

    // Intercept SAVE by aria-label/title
    ...
    // After UNDO by aria-label/title, restore dots visibility
    root.addEventListener('click', function(e){
