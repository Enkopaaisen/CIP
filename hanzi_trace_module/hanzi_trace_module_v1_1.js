...
/*! Hanzi Trace Module v1.1 (minimal)
 *  Adds: preloaded data support so you can load JSON via file inputs (works on file:// without server).
 */
(function(global){
  'use strict';
  ...
  function mount(container, options){
    ...
    function layout(strokes){
      gGrid.innerHTML=""; gFill.innerHTML=""; gTrace.innerHTML="";
    ...
    // draw even when starting outside tolerance; invalidate on release
    ...
    function endPress(){
      ...
      if(strokeDone){ strokeDone=false; setTimeout(()=>{ current=Math.min(current+1, basePaths.length-1); recolor(); }, opts.nextDelayMs|0); }
    }
    ...
    btns[0].onclick = ()=>{ // undo
      ...
      if(current>0){ current-=1; recolor(); if(tracePaths[current] && tracePaths[current].length){ const el=tracePaths[current].pop(); if(el&&el.parentNode) el.parentNode.removeChild(el); recomputeHits(current); } }
    };
    btns[1].onclick = ()=>{ // clear
      gTrace.innerHTML=""; tracePaths = tracePaths.map(()=>[]); hitSets = dots.map(()=> new Set()); current=0; recolor();
    };
    btns[2].onclick = ()=>{ // save
      const clone = svg.cloneNode(true);
      const embed = document.createElementNS(NS,"style"); embed.textContent = `.grid line, .grid rect{stroke:#cfcfcf;stroke-width:1.2;fill:none}.grid .border{stroke-width:1.5}.trace{fill:none;stroke:#000;stroke-linecap:round;stroke-linejoin:round}`;
    ...
    function setDots(d){ dots=(d&&Array.isArray(d.dots))? d.dots.map(a=> (a||[]).slice(0,MAX_DOTS)) : []; hitSets=dots.map(()=> new Set()); }
    ...
    else { initFromFetch().catch(err=>{ layout(["M 160 40 L 100 200","M 160 40 L 220 200"]); dots=[[{cx:100,cy:200},{cx:160,cy:40}],[{cx:160,cy:40},{cx:220,cy:200}]]; hitSets=dots.map(()=> new Set()); recolor(); console.warn("HanziTraceModule: using fallback; error:", err); }); }
