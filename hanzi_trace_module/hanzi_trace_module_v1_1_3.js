...
/*! Hanzi Trace Module v1.1.3
 *  Adds: numbered dot labels (1..N) rendered on top of dots.
 *  showDots: "off" | "active" | "always" (labels follow same visibility).
 */
(function(global){
  'use strict';
  ...
  function mount(container, options){
    ...
    const gDots  = svg.appendChild(el("g",{})); // rendered dots + labels
    ...
    function renderDots(){
      gDots.innerHTML=""; dotEls = dots.map((arr, si)=>{
        return (arr||[]).map((d, di)=>{
          ...
          // Append in order: halos below, then circles, then labels on top
    ...
    function updateDotVisibility(){
      ...
      dotEls.forEach((arr, si)=>{
        ...
        (arr||[]).forEach(el=>{
          ...
          el.halo.style.display = disp;
          el.circ.style.display = disp;
          el.label.style.display = disp;
        });
      });
    }
    ...
    btns[0].onclick = ()=>{ // undo
      if(tracePaths[current] && tracePaths[current].length){ const el=tracePaths[current].pop(); if(el&&el.parentNode) el.parentNode.removeChild(el); recomputeHits(current); updateDotVisibility(); return; }
...
// === Injected: Save trace to small 120x120 boxes (sg1..sg5) with fill-order logic ===
function _querySmallBoxes(){
  const ids = ['sg1','sg2','sg3','sg4','sg5'];
  const els = ids.map(id => document.getElementById(id)).filter(Boolean);
  return els;
}
function _firstEmptyBox(boxes){
  for (let i=0;i<boxes.length;i++){
    const el = boxes[i];
    const filled = (el.getAttribute('data-filled') === '1');
    if (!filled) return i;
  }
  return -1;
}
function _nextOverwriteIndex(){
  try{
...
function _advanceOverwriteIndex(){
  try{
    let n = parseInt(localStorage.getItem('smallBoxNext')||'1',10);
    if (!Number.isFinite(n) || n<1 || n>5) n = 1;
    n = (n % 5) + 1;
...
function _drawToSmallBox(canvasOrImage, targetCanvas){
  const ctx = targetCanvas.getContext('2d');
  const W = targetCanvas.width||120, H = targetCanvas.height||120;
  ...
  ctx.restore();
  const pad = 6; const dw = W - pad*2, dh = H - pad*2;
  ctx.drawImage(canvasOrImage, pad, pad, dw, dh);
  targetCanvas.setAttribute('data-filled','1');
}
function _snapshotTraceSVG(svgEl){
  ...
  const W = vb && vb.width ? vb.width : (svgEl.width.baseVal.value || 320);
  const H = vb && vb.height ? vb.height : (svgEl.height.baseVal.value || 320);
...
btns[2].onclick = ()=>{ // save
  const boxes = _querySmallBoxes();
  const svgEl = svg;
  if (boxes && boxes.length >= 1 && svgEl){
    _snapshotTraceSVG(svgEl).then((snap)=>{
      let idx = _firstEmptyBox(boxes);
      if (idx < 0){
        idx = _nextOverwriteIndex();
        _advanceOverwriteIndex();
      }
      const target = boxes[idx] || boxes[0];
      _drawToSmallBox(snap, target);
    });
    return;
  }
  // fallback to original download if no boxes
  const NS = "http://www.w3.org/2000/svg";
  ...
  const img = new Image();
  img.onload = function(){ const c=document.createElement("canvas"); c.width=W; c.height=H; const g=c.getContext("2d"); g.fillStyle="#ffffff"; g.fillRect(0,0,c.width,c.height); g.drawImage(img,0,0); URL.revokeObjectURL(url); downloadDataURL("trace_"+yyyymmdd_hhmmss()+".png", c.toDataURL("image/png")); };
  img.src = url;
};
